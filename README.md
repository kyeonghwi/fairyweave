...
- **2단계 생성 파이프라인** — 스토리 생성 후 `/story-review`에서 내용 검토·수정, 이후 이미지 생성
- **스토리 검토 화면** — 책 제목·페이지 텍스트 직접 편집, 페이지별 이미지 생성 토글, AI 지시어로 텍스트/이미지 프롬프트 재생성
...
- **2단계 생성 + 스토리 검토**: 스토리 생성 직후 `/story-review` 화면에서 제목·텍스트 편집, 페이지별 이미지 생성 선택, AI 지시어(예: "더 신나게", "강아지 추가") 기반 텍스트/이미지 프롬프트 재생성 후 이미지 생성 시작
- 부모가 완성된 책의 AI 생성 텍스트를 직접 수정하는 인라인 편집(co-creation) 기능

**사진 기반 초개인화 (Vision API 활용) 및 아동 보호(Kids-Safe) 설계 전략**
...
- 주문 상태 실시간 알림 UI
- 스토리 검토 화면에서 페이지 순서 재배치 및 페이지 추가/삭제
