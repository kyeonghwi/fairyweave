...
| Gemini 3.1 Pro | 아이디어 회의: 서비스 컨셉, 기능 범위, UX 플로우 브레인스토밍 | 2026-04-01 |
| Gemini 2.5 Flash | 더미 데이터 생성: 5권 × 16페이지 스토리 텍스트 및 이미지 프롬프트 생성 | 2026-04-02 |
| Stitch | UI 생성: 프론트엔드 페이지 레이아웃 및 컴포넌트 디자인 초안 생성 | 2026-04-02 |
...
| Claude Code | Phase 10 — Sweetbook API 확장 2차: GET /book-specs/:specUid(판형 상세+가격), GET/POST /books(목록/상태), PATCH /orders/:uid/shipping(배송지 수정), GET /templates/:uid(상세) | 2026-04-06 |
| Claude Code | Phase 11 — SQLite 영속 저장소 도입: better-sqlite3 설치, db.ts 싱글턴 + WAL 모드 초기화, bookStore.ts를 in-memory Map에서 SQLite prepared statements로 전면 교체 (save/get/list 인터페이스 유지, TTL 30분 + 최대 20권 정책 DB 레벨로 이식, progressTracker는 in-memory 유지) | 2026-04-06 |
| Claude Code | Phase 12 — 운영 대시보드 프론트엔드 구현: /dashboard 페이지 신설 (탭 2개), OrdersTab(주문 목록 5초 폴링 + 행 클릭 상세 + 취소 인라인 폼 + 배송지 수정), CreditsTab(잔액 카드 + 거래 내역 테이블), TopNavBar에 대시보드 링크 추가 | 2026-04-06 |
| Claude Code | Phase 13 — AI 이미지 캐릭터 일관성 전략 적용: (1) Gemini가 characterDescription 고정 문장(외모+액세서리) 생성 → 커버·전 페이지 imagePrompt에 자동 prepend, (2) 1/3 페이지에 뒷모습/와이드샷/실루엣 카메라 구도 지시, (3) GenerateStoryResult에 characterDescription 필드 추가 | 2026-04-07 |
...
### 시간이 더 있었다면 추가했을 기능
Gemini 레퍼런스 이미지로 캐릭터 일관성 강화, 주문 상태 실시간 알림 UI, 모바일 인라인 편집 UX 개선.
...
### 문항 1: 과제 수행 과정
Phase 1에서 Next.js 16 + Express 모노레포를 구성하고 Gemini 2.5 Flash API 연동을 검증했습니다. Phase 2에서 메타-프롬프트 엔지니어링으로 스토리 JSON 생성 파이프라인을 만들고, Promise.allSettled로 삽화를 병렬 생성했습니다. Phase 3에서 Sweetbook SDK로 5-step 책 생성(create → upload → cover → contents → finalize) + 주문 플로우를 구현했습니다. Phase 4에서 프론트엔드 UI 3페이지(홈/생성/책 프리뷰+주문)와 더미 데이터 5권을 완성했습니다. Phase 5에서 멀티 책 사양(3종), 비동기 생성+폴링 UX, 표지 이미지 분리, 이미지 동시생성 워커 풀, 한국어 현지화, 주문 가격 표시를 추가했습니다. Phase 6에서 실제 동화책처럼 보이는 BookViewer를 구현했습니다. 데스크탑은 좌(일러스트)/우(텍스트) 오픈북 스프레드에 CSS 3D 페이지 넘김 애니메이션을 적용했고, 모바일은 이미지 위에 텍스트를 오버레이하는 뷰로 전환됩니다. Phase 7에서 주문 전 크레딧 잔액을 POST /orders/estimate로 검증하는 로직을 추가하고, 책 사양 목록을 반환하는 GET /api/sweetbook/book-specs 엔드포인트를 구현했습니다. Phase 8에서 한국어/영어/이중언어 칩 UI를 추가하고, 이중언어 선택 시 Gemini가 text(한국어)와 textEn(영어)을 동시에 생성하여 책 뷰어에 두 언어를 함께 표시하는 기능을 구현했습니다. Phase 9에서 GET /book-specs API를 연동하여 판형 선택 UI를 동적 데이터로 전환하고, BookSpread를 실제 책 레이아웃(표지 → 첫 페이지 → image|text 펼침 구조)으로 재설계했습니다. 더미 데이터도 커버 이미지 포함 7페이지 구조로 정비했습니다. Phase 10에서 부모가 AI 생성 텍스트를 인라인 편집하여 인쇄에 반영하는 co-creation 기능을 추가하고, Sweetbook API 전체 엔드포인트(주문 목록/취소, 크레딧 잔액/거래내역, 책 목록/상태, 배송지 수정, 템플릿 조회)를 프록시했습니다. Phase 11에서 better-sqlite3로 SQLite 영속 저장소를 도입하여 서버 재시작 후에도 생성된 책이 유지되도록 했습니다.
...
가장 중요한 판단은 Gemini JSON 파싱 전략이었습니다. LLM 출력이 항상 완벽한 JSON이 아니라는 전제로, responseMimeType 강제 + 배열 추출 + trailing comma 제거의 3중 방어를 설계했습니다. 두 번째는 AI 생성 엔드포인트를 Next.js 프록시 대신 백엔드 직접 호출로 분리한 것입니다. 수 분 걸리는 요청에 프록시 타임아웃이 맞지 않았고, 장시간 요청만 분리하여 나머지 API는 프록시의 편의성을 유지했습니다. 세 번째는 generate-book을 비동기 폴링 방식으로 전환한 것입니다. bookId를 즉시 반환하고 백그라운드에서 생성하면서 progress 엔드포인트로 상태를 노출하여, 사용자가 긴 대기 시간 동안 진행 상황을 확인할 수 있게 했습니다. 네 번째는 템플릿 UID를 env var 하드코딩에서 API 동적 조회로 전환한 것입니다. 멀티 책 사양 지원 시 사양마다 env var를 관리하는 것은 확장성이 없어, 시작 시 API로 조회하되 env var를 fallback으로 두는 방식을 선택했습니다. 다섯 번째는 인라인 텍스트 편집을 books-from-data 통합 엔드포인트로 연결한 것입니다. 편집된 텍스트가 새 책 생성 → 주문까지 반영되려면 기존 AI 책 주문 경로를 재사용해야 했고, 수정된 페이지 데이터를 그대로 전달하는 방식으로 코드 중복 없이 통합했습니다. 여섯 번째는 SQLite 저장소 설계에서 인터페이스 유지를 최우선으로 한 것입니다. bookStore의 save/get/list 시그니처를 그대로 두고 내부 구현만 교체했기 때문에 라우트 파일을 한 줄도 수정하지 않았습니다. progressTracker는 진행 중 상태를 추적하는 용도이므로 서버 재시작 시 리셋이 올바른 동작이라 in-memory를 유지했습니다. 일곱 번째는 캐릭터 일관성 전략에서 모델 의존성을 낮추는 방향을 선택한 것입니다. ControlNet·Image-to-Image 같은 외부 파이프라인 없이, 프롬프트 엔지니어링만으로 일관성을 최대화했습니다. Gemini가 캐릭터 외모를 한 문장으로 고정하고, 모든 페이지에 자동 주입하며, 일부 페이지는 구조적으로 얼굴을 감추는(뒷모습/실루엣) 방식으로 AI의 얼굴 재현 한계를 회피했습니다.

### 문항 4: AI 도구 사용 중 겪은 실패 또는 문제
Gemini 2.0 Flash가 신규 계정에서 404를 반환하여 ListModels API로 사용 가능 모델을 확인한 뒤 2.5 Flash로 교체했습니다. LLM이 trailing comma가 포함된 JSON을 반환하여 파싱이 실패했고, responseMimeType 강제와 정규식 후처리로 해결했습니다. Next.js rewrites 프록시가 AI 생성의 긴 응답 시간에 ECONNRESET을 발생시켜, 해당 엔드포인트만 백엔드 직접 호출로 변경했습니다. base64 이미지가 sessionStorage 5MB 제한을 초과하여, window 객체에 임시 캐시하는 방식으로 페이지 간 데이터를 전달했습니다. Gemini 이미지 생성에서 글자/텍스트가 그림 안에 렌더링되는 문제가 발생하여, 이미지 프롬프트에서 텍스트/글자 관련 내용을 명시적으로 제외하는 필터를 추가했습니다.
...
| 2026-04-05 | Gemini 생성 이미지에 글자/텍스트가 렌더링됨 | 이미지 프롬프트에 스토리 텍스트가 포함되어 모델이 글자를 그림 안에 생성 | 이미지 프롬프트에서 텍스트/글자 관련 내용을 제외하는 필터 추가 |
