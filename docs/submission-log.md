...
### 시간이 더 있었다면 추가했을 기능
Gemini 레퍼런스 이미지로 캐릭터 일관성 강화, Webhook 기반 주문 상태 실시간 알림 UI(현재 서버 수신만 구현), 모바일 인라인 편집 UX 개선.
