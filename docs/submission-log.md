...
|---|-----------|----------|-----------|------|
| 1 | `npm run dev` 실행 | 프론트엔드(3000) + 백엔드(3001) 동시 시작 | | |
| 2 | http://localhost:3000 접속 | 홈페이지 로드, 더미 북 5권 갤러리 표시 | | |
| 3 | 더미 북 클릭 | 책 프리뷰 페이지 이동, 16페이지 슬라이더 표시 | | |
| 4 | 주문 폼 작성 (이름/주소/전화번호) | 폼 입력 정상 동작 | | |
| 5 | 주문 제출 | 완료 화면에 orderUid 표시 | | |
| 6 | 백엔드 콘솔 확인 | Sweetbook API 호출 로그 (creationType: TEST) | | |
